@if($programReservations->isEmpty())
    <div class="text-center text-muted py-4">لا توجد حجوزات برامج حالياً.</div>
@else
    @foreach($programReservations as $reservation)
        <div class="card mb-3 border-0 shadow-sm">
            <div class="card-body">
                <h5 class="text-primary mb-1">برنامج: {{ $reservation->program->title ?? 'تم حذف البرنامج' }}</h5>
                <p class="mb-1"><strong>الأخصائي:</strong> {{ $reservation->program->specialist->name ?? 'غير متوفر' }}</p>
                ...
                <div class="mt-3 text-end">
                    @if(auth()->user()->type !== 'specialist' && $reservation->status === 'pending')
                        <button class="btn btn-outline-danger btn-sm"
                            onclick="updateProgramReservation({{ $reservation->id }}, 'cancelled')">
                            إلغاء الحجز
                        </button>
                    @elseif(auth()->user()->type === 'specialist' && $reservation->status === 'pending')
                        <button class="btn btn-success btn-sm me-1"
                            onclick="updateProgramReservation({{ $reservation->id }}, 'confirmed')">
                            قبول
                        </button>
                        <button class="btn btn-outline-danger btn-sm"
                            onclick="updateProgramReservation({{ $reservation->id }}, 'cancelled')">
